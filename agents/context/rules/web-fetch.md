...
1. Built-in web search/fetch tool - Use the tool available in the current agent by default.
2. `ax` skill - Use its workflow for general URL fetching and extraction.
...
Pipe `ax "<API URL>" --body -f` into a `jq -e` filter that requires `.code == 200` and `.status != null` and emits only the needed fields. For API errors, report only `.code` and `(.message // .status.message)`; never expose the full response.
...
X Article URLs do not map directly to the API. Identify a containing post with the built-in web search or `agent-browser`, or report that it cannot be identified reliably.
