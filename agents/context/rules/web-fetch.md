...
2. curl fallback - Retry with `curl-fetch -fsSL -A "coding-agent/1.0" <url>`. This is a GET/HEAD-only HTTP(S) wrapper with a small option allowlist. If a required read-only fetch is unsupported, use `agent-browser` or request approval for an explicitly scoped raw `curl` command.
