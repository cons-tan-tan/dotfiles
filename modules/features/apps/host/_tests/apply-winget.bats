...
setup() {
  ...
  SCRIPT="$REPO_ROOT/modules/features/apps/host/_scripts/apply-winget.sh"
...
@test "Nix public app pins the Windows home and owner script" {
  if [[ -z ${APPLY_WINGET_PUBLIC_BIN:-} ]]; then
    skip "APPLY_WINGET_PUBLIC_BIN is only available in Linux Nix checks"
  fi

  local public_script
  public_script="$(readlink -f "$APPLY_WINGET_PUBLIC_BIN")"
  grep -Fqx "export APPLY_WINGET_WINDOWS_HOMEDIR=$APPLY_WINGET_EXPECTED_WINDOWS_HOMEDIR" \
    "$public_script"
  grep -F 'exec "$WINGET_BIN" configure \' "$public_script"
  grep -F '  -f "$WIN_CONFIG_PATH" "$@"' "$public_script"
}
