...
setup() {
  ...
  UPDATE_PINS_SMOKE_SCRIPT="$CI_SCRIPT_DIR/update_pins_smoke.sh"
...
@test "update pins smoke builds and executes the private contract" {
  local calls="$BATS_TEST_TMPDIR/nix-calls"
  local github_output="$BATS_TEST_TMPDIR/github-output"
  local live_cwd="$BATS_TEST_TMPDIR/live-cwd"
  local runner_temp="$BATS_TEST_TMPDIR/runner-temp"
  local smoke_executable="$BATS_TEST_TMPDIR/update-pins-smoke"
  local stub_dir="$BATS_TEST_TMPDIR/build-stubs"
  mkdir -p "$runner_temp" "$stub_dir"

  write_bash_stub "$stub_dir/nix" <<'SH'
printf 'nix' >"$UPDATE_PINS_CALLS"
printf ' <%s>' "$@" >>"$UPDATE_PINS_CALLS"
printf '\n' >>"$UPDATE_PINS_CALLS"
printf '/nix/store/00000000000000000000000000000000-update-pins-smoke\n'
SH
  write_bash_stub "$smoke_executable" <<'SH'
printf '%s\n' "$PWD" >"$UPDATE_PINS_LIVE_CWD"
SH

  run env \
    GITHUB_OUTPUT="$github_output" \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CALLS="$calls" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" build-private
  [ "$status" -eq 0 ]
  [ "$(<"$calls")" = 'nix <build> <.#checks.x86_64-linux.update-pins-smoke> <--no-link> <--no-write-lock-file> <--print-out-paths>' ]
  [ "$(<"$github_output")" = 'path=/nix/store/00000000000000000000000000000000-update-pins-smoke' ]

  run env \
    RUNNER_TEMP="$runner_temp" \
    UPDATE_PINS_LIVE_CWD="$live_cwd" \
    UPDATE_PINS_SMOKE="$smoke_executable" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" live-upstream
  [ "$status" -eq 0 ]
  [ "$(<"$live_cwd")" = "$runner_temp" ]
...
@test "update pins smoke creates an isolated detached checkout" {
  local calls="$BATS_TEST_TMPDIR/git-calls"
  local checkout="$BATS_TEST_TMPDIR/update-pins-check"
  local stub_dir="$BATS_TEST_TMPDIR/checkout-stubs"
  mkdir -p "$stub_dir"

  write_bash_stub "$stub_dir/git" <<'SH'
printf 'git' >>"$UPDATE_PINS_CALLS"
printf ' <%s>' "$@" >>"$UPDATE_PINS_CALLS"
printf '\n' >>"$UPDATE_PINS_CALLS"
if [[ ${1:-} == clone ]]; then
  mkdir -p "${!#}"
...
SH
  ...
  run env \
    ...
    GITHUB_WORKSPACE=/workspace/source \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CALLS="$calls" \
    UPDATE_PINS_CHECKOUT="$checkout" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" create-checkout
  [ "$status" -eq 0 ]
  [ "$(<"$calls")" = $'git <clone> <--no-local> <--no-checkout> <--single-branch> <--depth=1> <--no-tags> </workspace/source> <'"$checkout"$'>\ngit <-C> <'"$checkout"$'> <checkout> <--quiet> <--detach> <0123456789abcdef0123456789abcdef01234567>\ngit <-C> <'"$checkout"$'> <rev-parse> <HEAD>\ngit <-C> <'"$checkout"$'> <remote> <remove> <origin>\ngit <-C> <'"$checkout"$'> <status> <--short>' ]
}

@test "update pins smoke rejects unsafe checkout states" {
  local existing_checkout="$BATS_TEST_TMPDIR/existing-checkout"
  local git_calls="$BATS_TEST_TMPDIR/guard-git-calls"
  local mismatch_checkout="$BATS_TEST_TMPDIR/mismatch-checkout"
  local dirty_checkout="$BATS_TEST_TMPDIR/dirty-checkout"
  local stub_dir="$BATS_TEST_TMPDIR/checkout-guard-stubs"
  mkdir -p "$existing_checkout" "$stub_dir"
  printf 'preserve me\n' >"$existing_checkout/sentinel"
...
elif [[ ${1:-} == -C && ${3:-} == status && -n ${GIT_STATUS_OUTPUT:-} ]]; then
  printf '%s\n' "$GIT_STATUS_OUTPUT"
fi
SH
  ...
  run env \
    ...
    GIT_CALLS="$git_calls" \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CHECKOUT="$existing_checkout" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" create-checkout
  [ "$status" -eq 1 ]
  [ "$(<"$existing_checkout/sentinel")" = 'preserve me' ]
  [ ! -e "$git_calls" ]

  run env \
    GITHUB_SHA=0123456789abcdef0123456789abcdef01234567 \
    GITHUB_WORKSPACE=/workspace/source \
    GIT_REVISION=ffffffffffffffffffffffffffffffffffffffff \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CHECKOUT="$mismatch_checkout" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" create-checkout
  [ "$status" -eq 1 ]
  ...
  run env \
    GITHUB_SHA=0123456789abcdef0123456789abcdef01234567 \
    GITHUB_WORKSPACE=/workspace/source \
    GIT_STATUS_OUTPUT='?? generated-file' \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CHECKOUT="$dirty_checkout" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" create-checkout
  [ "$status" -eq 1 ]
}

@test "update pins smoke checkout is detached, clean, and has no remote" {
  local checkout="$BATS_TEST_TMPDIR/real-checkout"
  local source="$BATS_TEST_TMPDIR/source"
  local source_revision
  mkdir -p "$source"
  git -C "$source" init --quiet
  git -C "$source" config commit.gpgSign false
  git -C "$source" config user.email ci-test@example.invalid
  git -C "$source" config user.name 'CI test'
  printf 'fixture\n' >"$source/fixture"
  ...
  run env \
    ...
    bash "$UPDATE_PINS_SMOKE_SCRIPT" create-checkout
  [ "$status" -eq 0 ]
  [ "$(git -C "$checkout" rev-parse HEAD)" = "$source_revision" ]
  run git -C "$checkout" symbolic-ref --quiet HEAD
  [ "$status" -eq 1 ]
  [ -z "$(git -C "$checkout" remote)" ]
  [ -z "$(git -C "$checkout" status --short)" ]
}

@test "update pins smoke exercises one bounded updater and verifies both trees" {
  local calls="$BATS_TEST_TMPDIR/updater-calls"
  local checkout="$BATS_TEST_TMPDIR/update-pins-check"
  local stub_dir="$BATS_TEST_TMPDIR/updater-stubs"
  mkdir -p "$checkout" "$stub_dir"

  write_bash_stub "$stub_dir/timeout" <<'SH'
printf 'timeout' >>"$UPDATE_PINS_CALLS"
printf ' <%s>' "$@" >>"$UPDATE_PINS_CALLS"
printf '\n' >>"$UPDATE_PINS_CALLS"
SH
  write_bash_stub "$stub_dir/git" <<'SH'
...
SH
  ...
  run env \
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CALLS="$calls" \
    bash -c 'cd "$1" && exec bash "$2" exercise-updater' _ "$checkout" "$UPDATE_PINS_SMOKE_SCRIPT"
  [ "$status" -eq 0 ]
  [ "$(<"$calls")" = $'timeout <15m> <nix> <run> <.#update-pins> <--> <--force> <--check> <difit>\ngit <diff> <--exit-code>\ngit <status> <--short>' ]
  ...
  run env \
    ...
    PATH="$stub_dir:$PATH" \
    UPDATE_PINS_CALLS="$calls" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" verify-original
  [ "$status" -eq 0 ]
  [ "$(<"$calls")" = $'git <-C> </workspace/source> <diff> <--exit-code>\ngit <-C> </workspace/source> <status> <--short>' ]
}

@test "update pins smoke propagates updater failures and rejects a dirty original" {
  local calls="$BATS_TEST_TMPDIR/guard-calls"
  local checkout="$BATS_TEST_TMPDIR/update-pins-check"
  local stub_dir="$BATS_TEST_TMPDIR/updater-guard-stubs"
  mkdir -p "$checkout" "$stub_dir"
...
printf ' <%s>' "$@" >>"$UPDATE_PINS_CALLS"
printf '\n' >>"$UPDATE_PINS_CALLS"
exit "${TIMEOUT_STATUS:-0}"
SH
  write_bash_stub "$stub_dir/git" <<'SH'
if [[ ${1:-} == diff ]]; then
  exit "${GIT_DIFF_STATUS:-0}"
fi
if [[ ${1:-} == status || ${3:-} == status ]]; then
  printf '%s\n' "${GIT_STATUS_OUTPUT:-}"
fi
SH
  ...
  run env \
    GITHUB_WORKSPACE=/workspace/source \
    GIT_STATUS_OUTPUT=' M flake.lock' \
    PATH="$stub_dir:$PATH" \
    bash "$UPDATE_PINS_SMOKE_SCRIPT" verify-original
  [ "$status" -eq 1 ]
}

@test "update pins smoke rejects unknown operations" {
  run bash "$UPDATE_PINS_SMOKE_SCRIPT" unknown
  [ "$status" -eq 2 ]
  [[ "$output" == usage:* ]]
}
