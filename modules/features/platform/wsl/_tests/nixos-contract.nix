...
let
  ...
  cleanupRunner = pkgs.callPackage ../../nh/_packages/clean-user {
    nh = config.programs.nh.package;
    nix = config.nix.package;
    scope = "all";
  };
  resultRootPruner = pkgs.callPackage ../../nh/_packages/result-root-pruner { };
  growthStatePath = "/var/lib/${cleanupPolicy.growth.stateDirectory}";
  growthRunner = pkgs.callPackage ../../nh/_packages/clean-growth-runner {
