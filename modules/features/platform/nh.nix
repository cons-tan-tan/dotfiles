...
in
{
  features.platform-nh = {
    ...
    homeManager =
      ...
      let
        cleanupOwner = config.dotfiles.platform.nhCleanupOwner;
        enableUserCleanup = cleanupOwner == "home-manager";
        enableUserResultRootCleanup = cleanupOwner == "home-manager";
        ...
        userCleanupRunner = pkgs.callPackage ../../../nix/packages/nh-clean-user {
      ...
      in
      {
        programs.nh = {
          enable = true;
          clean = {
            # NixOS and standalone WSL own cleanup outside Home Manager.
            enable = enableUserCleanup;
            dates = if config.dotfiles.platform.environment == "darwin" then "weekly" else cleanupPolicy.dates;
    ...
    nixos =
      ...
      let
        growthChecker = pkgs.callPackage ../../../nix/packages/nix-store-growth-checker {
          nix = config.nix.package;
        };
        cleanupRunner = pkgs.callPackage ../../../nix/packages/nh-clean-user {
          nh = config.programs.nh.package;
          nix = config.nix.package;
          scope = "all";
        };
