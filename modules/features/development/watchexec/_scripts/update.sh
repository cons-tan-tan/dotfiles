...
pin_path=modules/features/development/watchexec/_overlays/pin.json
release_json=$(mktemp)
...
if [[ $tag != v"$version" || ${#version} -gt 128 || ! $version =~ ^[0-9]+(\.[0-9]+)+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$ ]]; then
  printf 'watchexec: unsupported release tag: %s\n' "$tag" >&2
  exit 1
fi
hashes='{}'

while IFS=$'\t' read -r system target; do
  asset_name="watchexec-${version}-${target}.tar.xz"
  ...
  [[ $asset_url == "$expected_url" ]] || {
    printf 'watchexec: unexpected asset URL for %s\n' "$asset_name" >&2
    exit 1
  }
  hash=$(nix store prefetch-file --json "$asset_url" | jq -er .hash)
  hashes=$(jq -c --arg system "$system" --arg hash "$hash" \
    '.[$system] = $hash' <<<"$hashes")
done < <(jq -er '.assets | to_entries[] | [.key, .value.target] | @tsv' "$pin_path")

jq --arg version "$version" --argjson hashes "$hashes" '
  .version = $version
