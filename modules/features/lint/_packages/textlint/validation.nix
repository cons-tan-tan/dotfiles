...
let
  ...
  runnerExecutable = lib.getExe runner.package;
...
pkgs.runCommand "textlint-app-validation"
  {
    nativeBuildInputs = [
      productionRunner.package
      runner.package
    ];
  }
  ''
    if ${runnerExecutable} > "$TMPDIR/no-args.stdout" 2> "$TMPDIR/no-args.stderr"; then
    ...
    ${runnerExecutable} --help > "$TMPDIR/help.stdout" 2> "$TMPDIR/help.stderr"
    test ! -s "$TMPDIR/help.stdout"
    grep -Fx 'usage: nix run dotfiles#textlint -- tech-jp <files...>' "$TMPDIR/help.stderr"

    if ${runnerExecutable} unknown file.md > "$TMPDIR/unknown.stdout" 2> "$TMPDIR/unknown.stderr"; then
    ...
    if ${runnerExecutable} tech-jp > "$TMPDIR/no-files.stdout" 2> "$TMPDIR/no-files.stderr"; then
    ...
    ${runnerExecutable} tech-jp 'docs/file with spaces.md'
    printf '%s\0' \
      --config \
    ...
    ${productionLintExecutable} --version > "$TMPDIR/production-version"
    test -s "$TMPDIR/production-version"
