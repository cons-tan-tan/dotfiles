...
let
  ...
  validateFixture =
    ...
    if !builtins.isAttrs fixture then
      ...
      then
        throw "${toString path} has invalid Module suite metadata"
  ...
  validateSuiteIdentities =
    ...
    if duplicatePaths != [ ] then
      throw "Module suite paths must be unique: ${builtins.toJSON duplicatePaths}"
    else if duplicateCheckNames != [ ] then
      throw "Module suite check names must be unique: ${builtins.toJSON duplicateCheckNames}"
    else
      null;
  ...
  mkCheck =
    ...
    let
      ...
      failureRunner = builtins.toFile "module-suite-failure-case.nix" ''
        { caseName }:
        let
          ...
          inputs = {
            flake-parts.flakeModules.modules = ${inputs.flake-parts.outPath} + "/extras/modules.nix";
      ...
      runFailureCases = lib.concatMapStringsSep "\n" (
        ...
        let
          checkExpectedFragments = lib.concatMapStringsSep "\n" (expectedFragment: ''
            if ! rg --fixed-strings --quiet -- ${lib.escapeShellArg expectedFragment} "$TMPDIR/${name}.diagnostic"; then
              printf 'unexpected Module suite diagnostic: %s\n' ${lib.escapeShellArg name} >&2
        ...
        ''
          if nix-instantiate \
            ...
            printf 'expected Module suite fixture to fail: %s\n' ${lib.escapeShellArg name} >&2
            exit 1
          fi
          ...
          ' "$TMPDIR/${name}.stderr" >"$TMPDIR/${name}.diagnostic"; then
            printf 'could not extract root Module suite diagnostic: %s\n' ${lib.escapeShellArg name} >&2
