...
let
  ...
  productionRunnerExecutable = lib.getExe productionRunner.package;
...
pkgs.runCommand "pptx-app-validation"
  ...
  ''
    ...
    ${runnerExecutable} pptx-validation-probe 'argument with spaces' tail
    ...
    ${productionRunnerExecutable} python -c 'import defusedxml, lxml, markitdown, PIL'
    ${productionRunnerExecutable} node -e "require('pptxgenjs'); require('sharp')"
