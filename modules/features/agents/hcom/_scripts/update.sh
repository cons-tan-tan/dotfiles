...
input_name=hcom-src
module_path=modules/features/agents/hcom/default.nix
pin_path=modules/features/agents/hcom/_packages/hcom/pin.json
release_json=$(mktemp)
...
if [[ $tag != v"$version" || ${#version} -gt 128 || ! $version =~ ^[0-9]+(\.[0-9]+)+(-[0-9A-Za-z.-]+)?(\+[0-9A-Za-z.-]+)?$ ]]; then
  printf 'hcom: unsupported release tag: %s\n' "$tag" >&2
  exit 1
fi
hashes='{}'
...
while IFS=$'\t' read -r system asset_name; do
  ...
  [[ $asset_url == "$expected_url" ]] || {
    printf 'hcom: unexpected asset URL for %s\n' "$asset_name" >&2
    exit 1
  }
...
[[ $(rg -F -c "$old_url" "$module_path") == 1 ]] || {
  printf 'hcom: expected one authoritative input URL\n' >&2
  exit 1
}
