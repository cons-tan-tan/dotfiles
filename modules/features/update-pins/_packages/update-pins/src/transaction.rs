...
mod tests {
    ...
    impl TestRepository {
        fn new() -> Self {
            ...
            std::fs::create_dir_all(
                directory
                    .path()
                    .join("modules/features/agents/hcom/_packages/hcom"),
            )
            .expect("hcom pin directory");
            std::fs::create_dir_all(
                directory
                    .path()
                    .join("modules/features/agents/browser/_packages/agent-browser"),
            )
            .expect("browser pin directory");
            std::fs::write(
                directory
                    .path()
                    .join("modules/features/agents/hcom/_packages/hcom/pin.json"),
                b"{\"hash\":\"old\"}\n",
            )
            .expect("pin");
    ...
    fn finalized_transactions_reject_writes_and_repeated_finalization() {
        ...
        assert!(matches!(
            transaction.remove("modules/features/agents/hcom/_packages/hcom/pin.json"),
            Err(UpdateError::TransactionFinalized)
        ));
        assert!(matches!(
            transaction.commit(),
    ...
    fn rollback_removes_created_files_and_restores_deleted_files() {
        ...
        let created = repository
            .path()
            .join("modules/features/agents/browser/_packages/agent-browser/pin.json");

        {
            ...
            transaction
                ...
                .expect("create pin");
            transaction
                .remove("modules/features/agents/hcom/_packages/hcom/pin.json")
                .expect("delete pin");
    ...
    fn scoped_transaction_ignores_unrelated_dirty_files_and_enforces_ownership() {
        ...
        let mut transaction = Transaction::begin_scoped(
            repository.repository(),
            &runner,
            ["modules/features/agents/hcom/_packages/hcom/pin.json"],
        )
    ...
    fn symlink_target_and_parent_are_rejected() {
        ...
        let browser_pin_parent = parent_repository
            .path()
            .join("modules/features/agents/browser/_packages/agent-browser");
        std::fs::remove_dir(&browser_pin_parent).expect("remove browser pin parent");
        symlink(outside.path(), &browser_pin_parent).expect("parent symlink");
        let mut transaction =
        ...
        assert!(!outside.path().join("pin.json").exists());
    }
    ...
    fn assert_no_staging_files(root: &Path) {
        fn assert_directory_has_no_staging_files(directory: &Path) {
            for entry in std::fs::read_dir(directory).expect("read repository directory") {
                    ...
                    .is_dir()
                {
                    if entry.file_name() != ".git" {
                        assert_directory_has_no_staging_files(&entry.path());
                    }
                } else {
                    assert!(
                        !entry
        ...
        assert_directory_has_no_staging_files(root);
    }
}
