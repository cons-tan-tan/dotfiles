...
const LOCAL_PACKAGE_EXPRESSION: &str = concat!(
    "(",
    include_str!("nix/local-package.nix"),
    r#") {
...
const LOCAL_CHECK_EXPRESSION: &str = concat!(
    "(",
    include_str!("nix/local-check.nix"),
    r#") {
  getEnv = builtins.getEnv;
  getFlake = builtins.getFlake;
  repoRoot = ./.;
...
const CANDIDATE_PACKAGE_EXPRESSION: &str = concat!(
    "(",
    include_str!("nix/candidate-package.nix"),
    r#") {
  getEnv = builtins.getEnv;
  getFlake = builtins.getFlake;
  repoRoot = ./.;
...
mod tests {
    ...
    fn package_selection_uses_environment_not_expression_interpolation() {
        ...
        assert!(
            LOCAL_PACKAGE_EXPRESSION
                .contains(r#"pkgs.dotfilesPackages.${getEnv "UPDATE_PINS_PACKAGE"}"#)
        );
        assert!(!LOCAL_PACKAGE_EXPRESSION.contains("shellfirm"));
    }

    #[test]
    fn package_build_expressions_use_feature_owned_package_set_builder() {
        for expression in [LOCAL_PACKAGE_EXPRESSION, CANDIDATE_PACKAGE_EXPRESSION] {
            assert!(expression.contains("/modules/features/nixpkgs/_interface"));
            assert!(!expression.contains("modules/features/nixpkgs/_lib"));
            assert!(!expression.contains("./nix/lib/mk-pkgs.nix"));
            assert!(!expression.contains("localPackageRegistry"));
            assert!(!expression.contains("watchexecOverlay"));
        }
    }
    ...
    fn check_selection_uses_environment_and_prints_one_output_path() {
        ...
        assert!(LOCAL_CHECK_EXPRESSION.contains("getEnv \"UPDATE_PINS_CHECK\""));
    ...
    fn candidate_build_injects_fake_hash_without_interpolating_pin_data() {
        ...
        assert!(CANDIDATE_PACKAGE_EXPRESSION.contains("/modules/features/update-pins/_interface"));
        assert!(CANDIDATE_PACKAGE_EXPRESSION.contains("updatePins.mkCandidatePackage"));
        assert!(!CANDIDATE_PACKAGE_EXPRESSION.contains("update-pins/_lib"));
        assert!(
            CANDIDATE_PACKAGE_EXPRESSION
                .contains(r#"getEnv "UPDATE_PINS_DEPENDENCY_PROVENANCE_JSON""#)
        );
