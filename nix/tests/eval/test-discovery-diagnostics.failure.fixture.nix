...
let
  ...
  ciCheck = import (repoRoot + "/nix/lib/ci-check.nix") { inherit lib; };
  testDiscovery = import (repoRoot + "/nix/tests/test-discovery.nix") { inherit lib; };
  composeUniqueChecks = import (repoRoot + "/nix/tests/compose.nix") { inherit lib; };
  validateBatsCatalog = import (repoRoot + "/nix/tests/bats/validate-catalog.nix") { inherit lib; };
  harness = import (repoRoot + "/nix/tests/eval/harness.nix") {
    inherit
      ciCheck
      lib
      repoRoot
